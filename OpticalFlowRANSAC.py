...
def computeMSE(prev, curr):
    ...
    (n,m)= np.shape(prev)
    # Mean Squared Error
    return np.sum(np.square(prev-curr))/(n*m)
...
def computeEntropy(img):
    elements = np.ndarray.flatten(img)
    unique_elements, counts_elements = np.unique(elements, return_counts=True)
    prob_elements = counts_elements/np.size(elements)
    
    # Entropy Value
    return -np.sum(prob_elements*np.log2(prob_elements))
...
def computeErrorImage(im1, im2):
    ...
    # Displayable Error Image
    display_res= np.zeros_like(res)
    display_res=np.minimum(255,np.maximum(0,res+128))   
    return display_res
...
def computeGMEError(flow, gme):
    err = np.sqrt(np.sum(np.square(gme-flow),1))
